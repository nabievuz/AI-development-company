...
Accepted Founder feedback accumulates as per-record learnings. The **distillation step**
converts those transient records into **durable role instructions** embedded in each
role's AGENTS.md template under a `## Learned` section, which is then reflected in
the live agent via `python3 scripts/gen_subagents.py`.

### When to distil
Run after ≥ 3 new accepted Founder corrections land for a role, or at every retro wave.

### How to distil (the pipeline — implemented in `scripts/memory_lib.py`)

1. **Recall** — fetch all accepted Founder feedback records for the role's project scope.
2. **Filter** — remove quarantined / expired / below-min-trust records (re-uses `recallable()`).
3. **Cluster** — group semantically similar learnings by token-Jaccard similarity
   (`cluster_learnings()`, default threshold 0.60). A cluster is a set of corroborating
   signals about the same underlying fact.
4. **Merge** — collapse each cluster into one higher-confidence insight (`merge_cluster()`):
   confidence boosts +1 per corroborating record beyond the first, capped at 10.
5. **Deny-boundary check** — records from `deny`-scoped projects are excluded before
   clustering. Distillation **never moves a learning across the workstream deny-boundary**.
6. **Manager gate** — a project-scoped insight may only be promoted to `org` scope by a
   manager. `needs_manager_gate()` flags the promotion; a non-manager agent must leave it
   at project scope and log an escalation rather than writing to `org`.
7. **Bound** — keep the top-`MAX_LEARNED` (default 10) entries by confidence (descending),
   then date (most recent first within the same tier). Hard cap — no unbounded growth.
8. **Apply** — write the bounded, dated `## Learned` section to the role's AGENTS.md overlay
   (`apply_learned_to_template()`). Each entry carries date, key, confidence, scope, and
   one-sentence insight.
9. **Regenerate** — re-run `python3 scripts/gen_subagents.py` so the updated overlay is
   the agent's live instructions on the next dispatch.

### Idempotency
Re-running distillation on the same records produces the same section (deterministic sort +
bound). Distillation replaces the previous `## Learned` section entirely — the record
history stays in ArcRift; the overlay carries only the current best view.
...
- **Distillation never moves a learning across a deny-boundary** and never promotes scope without
  a manager gate (`needs_manager_gate()` returns True → escalate, do not promote unilaterally).
- `## Learned` in a role template is machine-generated; human edits will be overwritten on next
  distillation run.
