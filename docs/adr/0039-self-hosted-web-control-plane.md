...
- **Status:** Accepted (Backend EM authored; **CTO ratified 2026-07-24 — RACI 3.1/3.6 A**; Security Lead consulted — auth/RBAC/audit; CDO consulted — dashboard UX)
...
- **Extends ADR 0028**; **ratified by the CTO on 2026-07-24** (RACI 3.1/3.6 A); Security Lead consulted on auth/RBAC/audit, CDO consulted on dashboard UX. CP-1…CP-6 judged sound and coherent against the Founder discovery answers (Q6 — Founder-only approval + read-only team) and the WS-E RBAC SSOT (`config/rbac.yaml` / `scripts/rbac.py`) this control plane MUST reuse: **CP-3/QONUN-5** — `gate.approve` is a Founder-identity permission, structurally founder-only (`load_grants` refuses any file granting it to a non-founder kind; `decide("agent:<role>", "gate.approve")` == deny by construction), so the dashboard, an agent, or any non-Founder role can NEVER sign a gate, and a GATE-5-open deployment stays machine-blocked regardless of any button (C4). **CP-5/NOT-a-daemon** — an optional, Founder-enabled, feature-flagged-OFF (`ws_h_control_plane`, confirmed present + `false` in `config/features.yaml`) process that degrades to the ADR-0028 static read cockpit and dispatches nothing itself. **CP-6/offline-install** — in-tenant only, no external SaaS; the vendored wheel bundle installs + boots with no network (verified on-device: fastapi/uvicorn/starlette/pydantic import clean, `/healthz` 200, arm64/cp310). **CP-4/C2** — all reads+writes route through the canonical board / goal queue / event store; the dashboard is a view+controller, never a source of truth. **Binding constraint carried to the Design/Development stage:** the control plane reuses the WS-E RBAC SSOT (`scripts/rbac.py`) — whose principal kinds are `founder` / `audit-team` / `agent` / `orchestrator` with `run.trigger` **Founder-only** and `audit-team` read-only — NOT the on-branch spike's ad-hoc `viewer<operator<founder` tier; and the spike's ~10 B008 (FastAPI `Depends`/`require` in argument defaults) ruff violations in `tools/control_plane/app.py` are cleaned as part of hardening it. No architectural defect found; `Proposed` → `Accepted` on this sign-off.
