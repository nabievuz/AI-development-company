...
| [0042](0042-adr0027-addendum-monthly-credit-ceiling.md) | **Addendum to [0027](0027-scheduler-safety.md) SI-5** (extends by reference; ADR-0027 stays `Accepted` and keeps SI-1…SI-7 — nothing superseded, nothing renumbered) — the Claude-subscription **monthly credit ceiling** as a binding OUTER dispatch cap, ratified against the shipped MUSTAQIL WS-F wiring (DAS-1618), not against its design note. Six subordinate clauses: SI-5.1 an outer cap **alongside, never in place of** `caps.per_run`/`caps.per_day` (tightest constraint wins), with `ws_b_admission.check_credit_exhaustion` the **sole** accountant (`loop_controller._monthly_credit_exhausted` is a no-arithmetic adapter, never routed via `admit()`/`gated_admit()`; `metered_overflow` stays OFF and Founder-only); **SI-5.2 the WINDOW, stated exhaustively** — the **UTC calendar month, start boundary INCLUSIVE**, anchored to an explicitly-threaded evaluation instant (`tick(now=…)` → `_window_start(now, unit="month")` → `aggregate_spans(since=…)`), rolling so a crossed ceiling **resets** instead of latching, one shared `_window_start` primitive for every SI-5 spend ceiling (a monthly limit vs a **lifetime** total was the caught defect: monotonic ⇒ permanent `idle` latch ⇒ the ≥3-day clean shadow window can never accumulate ⇒ go-live structurally unreachable); SI-5.3 exhaustion ⇒ action **`idle`** with `sanctioned_pause` as a **reason string, never a fourth action** — `flow_router.DECISIONS` stays the closed `{dispatch, validate, idle}` because that closure **is** SI-7's structural never-auto-approve enforcement (this addendum is explicitly **not** licence to widen it); SI-5.4 blocks **`dispatch` only, never `validate`**, never an error (exit 0, no raise), never a fabricated pause, surfaced in `safety_rails` in **shadow mode too**; SI-5.5 the `active_plan` resolution — **inert in the tick, blocking at the readiness gate** (failing closed in the tick would be a false red freezing the substrate, the same latch from the other direction; no agent may guess the plan — DAS-1629, Founder-gated); SI-5.6 a span with a **missing/unparseable `created_at` is EXCLUDED** from the window (counting it would put it in *every* window forever, reintroducing the latch), with the resulting silent-drop residual named and ticketed (DAS-1633). Open residuals recorded, not asserted away: per-day cap still lifetime (DAS-1632), undeclared `active_plan` (DAS-1629). The `idle_and_alert` limb — originally an open residual here — is now **wired and verified (DAS-1634, recorded by DAS-1642)**: `loop_controller.tick()` emits `alerting.sanctioned_pause_alert` (severity `info`, metric `SI-5`) as an observation alongside `idle`, adding no fourth action; its remaining **persistence residual** (in-band on the tick surface, no monitored sink outside `tick()`) is deferred to DAS-1643. | Accepted | 2026-07-24 |
...
- **Autonomous tempo — ORGANISM WS4 HEARTBEAT ([0027](0027-scheduler-safety.md)–).**
  ...
  **[0042](0042-adr0027-addendum-monthly-credit-ceiling.md) is an addendum to SI-5**
  (read as part of it; ADR-0027 itself is unedited): the Claude-subscription
  **monthly credit ceiling** is a binding **outer** dispatch cap alongside the
  per-run/per-day caps, measured over the **UTC calendar month with an inclusive
  start boundary** (never a lifetime total — a lifetime total latches at `idle`
  permanently and makes go-live structurally unreachable), blocking `dispatch`
  only and never `validate`, producing the action `idle` with `sanctioned_pause`
  as a **reason string and never a fourth decision action** (the closed
  `{dispatch, validate, idle}` alphabet *is* SI-7's structural enforcement), with
  an undeclared `active_plan` inert in the tick but blocking at the readiness gate
  and undated spans excluded from the window.
...
- **LangChain-ecosystem interop / Governed-Devin direction ([0033](0033-ecosystem-tool-mcp-bridge.md)–[0040](0040-a2a-outbound-surface.md)).** The direction that brings DasLab to autonomous-agent-platform parity while keeping governance as the moat (briefs: `docs/research/2026-07-22-daslab-devin-langchain-direction.md`, `docs/research/2026-07-22-daslab-vs-autonomous-coding-agents-parity.md`). The LangChain ecosystem is adopted strictly as **substrate under DGO-X** (ADR 0010 C1): an inbound MCP tool bridge for browser/computer-use + the integration catalog (0033), a Claude Agent SDK headless runner that realizes ADR 0009's deferred SDK gateway (0034), LangGraph as the DGO-X P2/P3 execution engine with AADL gates as conditional edges/interrupts (0035), and an outbound subgraph/MCP surface plus a non-invasive **self-host Langfuse** OTLP observability lens (0036 — self-host/in-tenant only, NOT LangSmith), and an **A2A outbound surface** (0040) that extends 0036 so *another agent system* can call DasLab as a governed unit — a caller submits a **goal proposal, never a gate approval** (approvals stay Founder-only), the surface is in-tenant only, caller input is untrusted, and publishing the endpoint is a Founder act (dedicated `a2a_outbound` flag OFF; Q12 recorded as a go-live gate). All are feature-flagged OFF and change no dispatch behaviour on merge (0033/0034/0036/0040 Accepted; 0035 Proposed). The MUSTAQIL v3.0 prep layer adds an explicit **retrieval-strategy** decision ([0041](0041-agentic-search-first-retrieval-strategy.md), ratifying Founder Q11): agentic-search-first (grep / Read / `07-CONTEXT-PACK` / ArcRift recall) is the default and no vector DB is stood up by default; an indexed-retrieval escape hatch is built only when a large-repo metric *and* a governing ADR both approve it, and if built it enters as a derived, governed tool through the ADR 0033 edge — the index is never canonical, `board/tickets/` stays the source of truth (C2).
